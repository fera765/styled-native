...
import { calculate, min, max } from './utils/maths'
...
export function convertValue(key: string, value: string, units: Units): string | number {
  ...
  if (value.includes('px')) {
    ...
    if (['marginTop', 'marginBottom'].includes(key)) finalUnits['vh'] = units.height! / 100
    else if (['marginLeft', 'marginRight'].includes(key)) finalUnits['vh'] = units.width! / 100
    else if (key.startsWith('border') && key.endsWith('Radius')) finalUnits['vh'] = (units.width! + units.height!) / 200
    else if (['width', 'height', 'minWidth', 'minHeight', 'maxWidth', 'maxHeight', 'top', 'left', 'bottom', 'right', 'flexBasis', 'rotate3d'].includes(key)) {

      if (value.startsWith('calc') || value.startsWith('max') || value.startsWith('min')) {
        if (['height', 'minHeight', 'maxHeight', 'top', 'bottom'].includes(key)) finalUnits['vh'] = units.height! / 100
        else finalUnits['vh'] = units.width! / 100

      }
      // width: 100%, height: 100% are supported
      else return value
    }
    else if (['lineHeight'].includes(key)) finalUnits['vh'] = units.em / 100
    else finalUnits['vh'] = 0.01
  } else if (value.includes('vw')) {
    if (['marginTop', 'marginBottom'].includes(key)) finalUnits['vw'] = units.height! / 100
    else if (['marginLeft', 'marginRight'].includes(key)) finalUnits['vw'] = units.width! / 100
    else if (key.startsWith('border') && key.endsWith('Radius')) finalUnits['vw'] = (units.width! + units.height!) / 200
    else if (['width', 'height', 'minWidth', 'minHeight', 'maxWidth', 'maxHeight', 'top', 'left', 'bottom', 'right', 'flexBasis', 'rotate3d'].includes(key)) {

      if (value.startsWith('calc') || value.startsWith('max') || value.startsWith('min')) {
        if (['height', 'minHeight', 'maxHeight', 'top', 'bottom'].includes(key)) finalUnits['vw'] = units.height! / 100
        else finalUnits['vw'] = units.width! / 100

      }
      // width: 100%, height: 100% are supported
      else return value
    }
    else if (['lineHeight'].includes(key)) finalUnits['vw'] = units.em / 100
    else finalUnits['vw'] = 0.01
  }
