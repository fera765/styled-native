...
const varRegExp = /var\s*\(\s*--([_a-z0-9-]+)\s*(?:,\s*([^)]+))?\)/g;
    ...
    styleObject = Object.assign(
      ...
      ...cssDeclaration
        ...
        .map(([name, value]) => {
          ...
          let destinationVariables = resolverLengsVariables;

          if (colorAttributes.has(getPropertyName(name))) {
            destinationVariables = resolveColorVariablePlaceholder;
          } else if (lengthAttributes.has(getPropertyName(name))) {
            destinationVariables = resolverLengsVariables;
          }

          if (value.match(varRegExp)) {
            resolvedValue = value.replace(varRegExp, (_, variable: string, fallback): string => {
              if (fallback) {
                return destinationVariables(`$${variable}`, fallback);
              }
              return destinationVariables(`$${variable}`);
            });
          } else if (value.match(variableRegexp)) {
            resolvedValue = value.replace(variableRegexp, (variable: string): string =>
              destinationVariables(variable)
            );
          }
...
const useStyleSheet = (
  ...
  return useMemo(() => {
    ...
    for (const key in stylesCopy) {
      ...
      const mediaStylesArray = [
        resolveThemeVariables({ ...main }, theme, windowDimensions, units.current),
      ];
      for (const mediaRule in mediaStylesCopy) {
        if (matchMediaRule(mediaRule, theme, windowDimensions)) {
          mediaStylesArray.push(
            resolveThemeVariables(
              { ...mediaStylesCopy[mediaRule] },
              theme,
              windowDimensions,
              units.current
            )
...
>(
  ...
  ...expressions: TemplateStringExpression<AttrProps<P, I, A> & A>[]
    ...
    const displayName = hashGenerate();
...
export const css = (cssDeclaration: TemplateStringsArray): string => {
  return String(cssDeclaration);
};
